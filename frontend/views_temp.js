...
// ============ DUE DILIGENCE FORM - PART 1 ============
function dueDiligenceFormPart1(client, formData, user, errors = null, clientId = null) {
  const d = formData || {};
  const submitted = !!d._submitted;
  const approvalStatus = d._approval_status || 'pending';
  const readOnly = submitted ? 'disabled' : '';
  const formAction = '/client/due-diligence/part1';
  ...
  const content = `
    ...
    <div class="container">
      <div class="card">
        <div class="card-header">
          <h2>Due Diligence Form - Part 1</h2>
          <div>
        ...
        <div style="padding: 20px; background: rgba(76, 175, 80, 0.1); border-radius: 8px; margin: 20px; border-left: 4px solid var(--color-neon-green);">
          <p style="margin: 0; color: var(--color-text);">
            <strong style="color: var(--color-neon-green);">📄 Part 1:</strong> Enterprise / Partnership / Company / Individual
          </p>
        </div>
        
        <div class="accordion-header" onclick="toggleAccordion('duediligence')">
          <h3>📋 Due Diligence Form - Part 1</h3>
        ...
        <form method="POST" action="${formAction}" class="form-light">
           <input type="hidden" name="entity_type_selection" value="part1">
  ...
  `;
  return htmlPage('Due Diligence Form - Part 1', content, user, clientId);
}

// ============ DUE DILIGENCE FORM - PART 2 ============
    ...
    <div class="container">
      <div class="card">
        <div class="card-header">
          ...
          <div>${submitted ? statusBadges[approvalStatus] || statusBadges['pending'] : '<span class="badge badge-pending">Draft</span>'}</div>
        </div>
        ${passwordChangeBanner(user)}
        ${errors ? '<div class="alert alert-error">' + errors + '</div>' : ''}
        ...
        <div style="padding: 20px; background: rgba(135, 206, 250, 0.1); border-radius: 8px; margin: 20px; border-left: 4px solid var(--color-sky-blue);">
          <p style="margin: 0;"><strong style="color: var(--color-sky-blue);">📋 Part 2:</strong> Government / Clubs / Societies / Schools / Universities / Embassies</p>
        </div>
        ...
        <form method="POST" action="${formAction}" class="form-light">
          ...
          <div class="form-grid-full"><div class="form-group">
            ...
            <div style="font-size: 0.85em; color: var(--color-text-muted); margin-top: 6px;">
              <strong>Note:</strong> Please state name of ministry/government department/government agencies/state government/club/society etc
            </div>
          </div></div>
          <div class="form-grid-2"><div class="form-group"><label>Registration No.</label><input type="text" name="entity_reg_no" value="${val('entity_reg_no')}" ${readOnly}></div><div class="form-group"><label>Tax ID (TIN)</label><input type="text" name="entity_tin" value="${val('entity_tin')}" ${readOnly}></div></div>
          <div class="form-grid-2"><div class="form-group"><label>SST Registration No.</label><input type="text" name="entity_sst" value="${val('entity_sst')}" ${readOnly}></div><div class="form-group"><label>Date of Registration</label><input type="date" name="entity_date_registration" value="${val('entity_date_registration')}" ${readOnly}></div></div>
          <div class="form-grid-2"><div class="form-group"><label>Country</label><input type="text" name="entity_country_registration" value="${val('entity_country_registration')}" ${readOnly}></div><div class="form-group"><label class="required">Contact no.</label><input type="text" name="entity_contact_no" value="${val('entity_contact_no')}" ${readOnly} required></div></div>
          <div class="form-grid-full"><div class="form-group"><label class="required">Registered Address / Business Address (whichever is applicable)</label><textarea name="entity_registered_address" rows="2" ${readOnly} required>${val('entity_registered_address')}</textarea></div></div>
          <div class="form-grid-2"><div class="form-group"><label class="required">Email</label><input type="email" name="entity_email" value="${val('entity_email')}" ${readOnly} required></div><div class="form-group"><label>Contact Email</label><input type="email" name="entity_contact_email" value="${val('entity_contact_email')}" ${readOnly}></div></div>
                    <div class="form-grid-full"><div class="form-group">
